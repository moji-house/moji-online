...
export async function uploadToCloudinary(file, folder = 'real-estate', resourceType = 'auto') {
  try {
    if (!file) {
...
export async function uploadVideo(file, folder = 'real-estate/videos') {
  return uploadToCloudinary(file, folder, 'video');
}

// ฟังก์ชันสำหรับอัพโหลด PDF
export async function uploadPDF(file, folder = 'real-estate/documents') {
