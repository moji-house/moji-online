...
export default function PointDisplay({ propertyId, initialPoints = 0 }) {
      ...
      if (session?.user?.id) {
        try {
          const response = await axios.get(
            `/api/properties/${propertyId}/vote/check`,
            {
  ...
  const handleVote = async (voteType) => {
    ...
    try {
      ...
      toast.error(
        error.response?.data?.error || "ไม่สามารถโหวตได้ กรุณาลองใหม่อีกครั้ง"
      );
    } finally {
      setIsVoting(false);
    }
  ...
  return (
    <div className="flex items-center space-x-2 bg-white p-2 rounded-lg shadow-sm">
      ...
      >
        <FaChevronUp
          className={`${userVoted ? "text-gray-400" : "text-green-600"}`}
        />
      </button>
      ...
      >
        <FaChevronDown
          className={`${userVoted ? "text-gray-400" : "text-red-600"}`}
        />
      </button>
    </div>
  );
