...
export default function PropertyCard({ property }: { property: ISerializedProperty }) {
  const { data: session } = useSession();
  const typedSession = session as UserSession;
  const router = useRouter();
  const [propertyData, setPropertyData] = useState<ISerializedProperty>(property);
    ...
    const fetchPropertyData = async () => {
      try {
        ...
        if (response.ok) {
          ...
          if (userProfile?.id) {
            const isUserLiked = property.likes?.some(
  ...
  }, [property?.id, typedSession?.user?.id]);
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full transform hover:-translate-y-1 hover:scale-[1.01] transition-transform">
      <PointsDisplay propertyData={propertyData} />
      ...
      {propertyData.images && propertyData.images.length > 0 && (
        <div className="relative h-48 sm:h-56 md:h-64 w-full">
          ...
          <div className="absolute top-3 left-3 z-10">
            <span
              className={`px-3 py-1 rounded-full text-xs font-semibold ${propertyData.status === "for sale"
