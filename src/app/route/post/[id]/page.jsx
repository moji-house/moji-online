...
export default function PropertyCard() {
    ...
    const fetchProperty = async () => {
      try {
        ...
        const owner = userProfiles.find(
          (profile) => profile.id === response.data.userId
        );
        ...
        if (userProfile?.id) {
          const isUserLiked = response.data.likes?.some(
  ...
  const handleLikeClick = async (e) => {
    ...
    try {
      setIsLoading(true);
      const response = await axios.post(`/api/properties/${property.id}/like`, {
  ...
  const displayImages = hasImages ? property.images.slice(0, 3) : [];
  ...
  return (
    <div className="bg-white rounded-xl overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col h-full transform hover:-translate-y-1 hover:scale-[1.01] transition-transform">
      <PointsDisplay points={property.points} propertyData={property} />
